...
func TestUpdateCorrespondent(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
	...
	}{
		{
			...
			setup: func() {
				pact.
					AddInteraction().
					Given("I have an EPA with actor details").
					UponReceiving("A request to update a correspondent").
					WithCompleteRequest(consumer.Request{
						Method: http.MethodPut,
						Path:   matchers.String("/lpa-api/v1/persons/654"),
						Headers: matchers.MapMatcher{
							"Content-Type": matchers.String("application/json"),
						},
						Body: map[string]interface{}{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Nil(t, pact.ExecuteTest(t, func(config consumer.MockServerConfig) error {
				...
				err := client.UpdateCorrespondent(Context{Context: context.Background()}, 654, tc.correspondent)
				if tc.expectedError == nil {
					assert.Nil(t, err)
				} else {
					assert.Equal(t, tc.expectedError(config.Port), err)
