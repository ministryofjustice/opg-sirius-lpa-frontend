...
func TestDocumentTemplateIdRefDataIgnoredPact(t *testing.T) {
	// We need this test to produce a specific array of document template ref data in the
	...
	}{
		{
			...
			setup: func() {
				pact.
					AddInteraction().
					Given("Some document template ids types exist").
					UponReceiving("A request to get all the document template ref data I need").
					WithRequest(dsl.Request{
						Method: http.MethodGet,
						Path:   dsl.String(fmt.Sprintf("/lpa-api/v1/reference-data/%s", DocumentTemplateIdCategory)),
					}).
