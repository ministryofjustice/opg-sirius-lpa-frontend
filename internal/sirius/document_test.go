...
func TestOutgoingDocumentBySystemType(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Nil(t, pact.ExecuteTest(t, func(config consumer.MockServerConfig) error {
				...
				document, err := client.OutgoingDocumentBySystemType(Context{Context: context.Background()}, tc.caseType, 800, "LP-A")
