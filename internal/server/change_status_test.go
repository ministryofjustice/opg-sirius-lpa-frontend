...
func (m *mockChangeStatusClient) GetAllowedStatuses(ctx sirius.Context, caseID int, caseType sirius.CaseType) ([]string, error) {
...
func TestGetChangeStatus(t *testing.T) {
	for _, caseType := range []string{"lpa", "epa"} {
		t.Run(caseType, func(t *testing.T) {
			...
			client.
				On("GetAllowedStatuses", mock.Anything, 123, sirius.CaseType(caseType)).
			...
			template.
				On("Func", mock.Anything, changeStatusData{
					...
					AllowedStatuses: []string{"Cancelled", "Withdrawn"},
				}).
				Return(nil)
...
func TestGetChangeStatusWhenAvailableStatusesErrors(t *testing.T) {
	...
	client.
		On("GetAllowedStatuses", mock.Anything, 123, sirius.CaseTypeLpa).
