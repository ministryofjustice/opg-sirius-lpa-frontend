...
func DeleteDocument(client DeleteDocumentClient, tmpl template.Template) Handler {
	return func(w http.ResponseWriter, r *http.Request) error {
		...
		if r.Method == http.MethodPost {
			...
			return RedirectError(fmt.Sprintf("/donor/%d/documents?success=true&documentFriendlyName=%s&documentCreatedTime=%s&uid[]=%s", donorId, documentData.FriendlyDescription, documentData.CreatedDate, documentData.CaseItems[0].UID))
		}
