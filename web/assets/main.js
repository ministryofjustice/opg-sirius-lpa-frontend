...
if (selectUser) {
    accessibleAutocomplete.enhanceSelectElement({
        ...
        templates: {
            inputValue(value) { return !value ? '' : value.text },
            suggestion(value) { return value.text },
        },
        onConfirm(selected) {
            if (selected) {
                selectUser.innerHTML = `<option value="${selected.id}:${selected.text}">${selected.text}</option>`;
            }
        },
    });
}
